    ...
    def train(self, train_dataloader, config, test_dataloader=None):
        ...
        for epoch in trange(0, config.num_epochs, desc=f"[Full Loop]", leave=False):
            ...
            for inp, label, _ in tqdm(train_dataloader, desc=f"[Train]", leave=False):
                ...
                # ssim_loss_fn = pytorch_ssim.SSIM().to(device)
            ...
            if epoch % config.print_freq == 0:
                print('epoch:[{}]/[{}], image loss:{}, MSE / L1 loss:{}, VGG loss:{}, SSIM loss:{}'.format(epoch, config.num_epochs,
                                                                                             str(total_loss_lst[epoch]),
                                                                                             str(primary_loss_lst[
                                                                                                     epoch]),
                                                                                             str(vgg_loss_lst[epoch]),
                                                                                             str(ssim_loss_lst[epoch])))
                # wandb.log()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--snapshots_folder', type=str, default="./SnapShot/SSIMLossAdd")
    parser.add_argument('--output_images_path', type=str, default="./ValImg/SSIMLoss/")
